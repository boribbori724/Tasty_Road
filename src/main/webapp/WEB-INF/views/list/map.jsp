...
<body class="MAP">
	...
	<div class="IE6MIN">
		<div id="header" class="Header" role="banner" style="padding: 0px;">
			<h3 class="Title"><i class="fas fa-road"></i>Tasty_Road</h3> 
			<form>
					...
					<ul class="menu">
						...
						<c:if test="${!empty login }">
							<li id="search.tab5" class="favorite favorite-INACTIVE login"><a
								href="/member/myPage.do" class="mainmenutab" title="즐겨찾기">${login.id }</a></li>
						</c:if>
					</ul>
				</div>
			</div>
...
//       alert(${vo.address});
        ...
        var listData = [
           //["addr0","addr1"]
            ["서울 강남구 언주로30길 26","서경그룹"],
            ["경기 의정부시 신흥로 251-1","차이나푸드"],
            ["경기 의정부시 신흥로258번길 21","골목식당"],
            ["경기 의정부시 신흥로257번길 6","서브웨이"],
            ["경기 의정부시 시민로 80","kfc"]
        ];
